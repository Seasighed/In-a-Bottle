...
- `Scripts/UI/SurveyRuntimeBuildProfile.gd` resolves participant versus QA behavior at runtime for both shells.
...
- `tools/playtest.ps1` is the repo-owned playtest pipeline entrypoint for local operators and CI.
...
5. Runtime profile resolution applies before the shells finish startup, so the same exported artifact can intentionally boot in `participant` or `qa` mode without editing tracked defaults.
6. Build exports use `SurveyBuildExportSupport` to normalize the chosen builds folder, inspect existing version folders, pick the next `v###` slot, compute preset-specific target paths, and verify the artifacts that should exist after export.
7. `tools/playtest.ps1` wraps repo validation, export packaging, and visual-audit export into one local and CI workflow.
8. The tooling scene and CLI block known-bad export combinations before they start. Right now the important guard is `Web + Mono Godot editor`, because the repo vendors SeaShell C# addon files and Godot 4 Mono will not produce a valid Web export from that setup.
9. The addon settings, feature flags, and runtime build profiles let the project ship a simpler participant-facing surface while keeping QA or editor tooling available only when explicitly enabled.

## Operational Notes

- `localBuilds/` is generated output and should stay out of git history.
- `build/playtest/` is the new versioned handoff root for playtest packaging and should also stay local.
...
- CI now calls the playtest CLI directly, with Linux responsible for Web export and Windows responsible for desktop export. Pages deployment should publish only participant Web output.
...
- 2026-06-17 04:35 - Added runtime build profile, playtest CLI, versioned playtest packaging, and participant-only Pages deployment notes.
