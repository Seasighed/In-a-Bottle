...
| --- | --- | --- | --- | --- | --- | --- | --- |
| QA-001 | High | Local Git | Worktree curation | Create a branch/commits in the foreground checkout, then use the safety clone fallback. | Foreground `.git` lock creation is still denied, but clean branch `codex-release-candidate-proof-pass` was pushed to GitHub. | Mitigated | No |
...
| QA-003 | High | Renderer-backed Godot | Visual proof | Run `tools/playtest.ps1 release -IncludeAudit` from the promoted branch. | Final promoted-branch audit under `build/playtest/20260630-promoted-branch-proof/audit/` has 92 captures and 0 placeholders. | Fixed | No |
...
- 2026-06-30 21:21 - Recorded the promoted GitHub branch, final promoted-branch release proof path, working Supabase CLI via `npx`, and fixed Deno validation finding.
